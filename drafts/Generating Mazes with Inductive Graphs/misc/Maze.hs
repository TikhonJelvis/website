...
generate :: MonadRandom m => Int -> Int -> m [Graph.Edge]
generate width height = liftM (Graph.edges graph \\) $ edfsR (ghead graph) graph
...
-- | Convert a list of unlabeled edges into just their labels. This is
--   used to generate a list of walls to draw from the list of edges
--   generated by @generate@.
toWalls :: Grid -> [Graph.Edge] -> [Wall]
toWalls grid = map (edgeLabel grid)


-- | Generates a random n × m maze.
maze :: MonadRandom m => Int -> Int -> m [Wall]
maze width height = liftM (toWalls $ grid width height) $ generate width height
